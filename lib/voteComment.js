...
module.exports = async (commentId, vote, pgdb, user, req, t) => {
  if(vote !== 1 && vote !== -1) {
    logger.error('voteComment: vote out of range', { req: req._log(), commentId, vote })
  ...
  try {
    ...
    if(!comment) {
      logger.error('voteComment: comment not found', { req: req._log(), commentId, vote })
    ...
    if(!membership) {
      logger.error('voteComment: membership required', { req: req._log(), commentId, vote })
