...
exports.publishComment = (user, comment, oldComment) => {
  let content
  if(oldComment) {
    content = `*${user.firstName} ${user.lastName}* edited: (${comment.id.substring(0, 8)})\n\n*old:*\n${oldComment.content}\n\n*new:*\n${comment.content}`
  } else {
    content = `*${user.firstName} ${user.lastName}* wrote: (${comment.id.substring(0, 8)})\n\n${comment.content}`
  }
